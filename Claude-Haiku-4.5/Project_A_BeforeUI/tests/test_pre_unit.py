"""Unit tests for Project A (Before) - Layout validation and DOM structure."""
...
class TestSpacingConsistency:
    ...
    def test_inconsistent_spacing(self):
        """Test detection of inconsistent spacing."""
        items = [
            {'padding': '12px', 'margin': '0px'},
        ...
        passed, errors = validate_spacing_consistency(items, tolerance_px=2.0)
        assert not passed
        assert any('variance' in str(e).lower() for e in errors)
