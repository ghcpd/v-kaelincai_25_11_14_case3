...
# Unit test helpers to calculate spacing and alignment metrics from bounding boxes
...
def spacing_variance(gaps: List[float]) -> float:
    ...
    return var**0.5  # standard deviation
...
def normalize_dom_tree(tree: Dict) -> Dict:
    # ensure no nulls, strings exist, and normalize missing titles
    def walk(node):
...
def alignment_issues(icon_boxes: List[Dict], texts: List[Dict]) -> int:
    # count how many icons are not aligned to baseline of text
    issues = 0
    for ib, tb in zip(icon_boxes, texts):
        # baseline approximate: icon center y vs text first-line y
