...
def compute_metrics(page):
    ...
    # hover feedback latency: add small test: dispatch mousemove and check computed background transition timing
...
def save_results(results, fname='results_pre.json'):
    ...
    try:
        with sync_playwright() as pw:
            ...
            for tc in td:
                ...
                if tc.get('hover'):
                    # record time to hover
                    page.hover('.btn')
                if tc.get('css_collision'):
                    # inject a conflicting global css rule (simulating collision)
                ...
                # Evaluate expected pass/fail from test_data
                ...
                if 'css_collisions_resolved' in expect:
                    # in pre, we expect collisions NOT resolved
                    pass_results['css_collisions_resolved'] = False if tc.get('css_collision') else True
                metrics['pass'] = pass_results
                results[tc['id']] = metrics
            save_results(results)
            with open(os.path.join(ROOT, 'results', 'time_pre.txt'), 'w') as tf:
