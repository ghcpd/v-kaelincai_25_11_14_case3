...
def test_button_height_consistency():
    heights = [36, 36, 34, 36]
    diff = check_button_height_consistency(heights)
    assert diff <= 2


def test_dom_normalizer_handles_nulls():
    raw = {'page':'task_list','items':[{'title':None}, {'title':'OK'}]}
    norm = normalize_dom_tree(raw)
    assert norm['items'][0]['title'] == '(untitled)'


def test_alignment_issues():
    icons = [{'x':10,'y':20,'width':20,'height':20}, {'x':10,'y':30,'width':20,'height':20}]
    texts = [{'x':40,'y':30,'width':200,'height':20}, {'x':40,'y':30,'width':200,'height':20}]
    issues = alignment_issues(icons, texts)
    assert issues >= 0


def test_dom_mutation_normalize():
